...
const FAQ_BANK = [
  ...
  { id:'comando-simplificar', keywords:['simplificar','comando simplificar','que hace simplificar'],
    answer:'"simplificar" hace tres cosas automáticas: reparte una potencia sobre una fracción o un producto (ej. $(a/b)^2 \\to a^2/b^2$), cancela una fracción cuando el numerador y el denominador son exactamente iguales (ej. $a/a \\to 1$), y convierte un producto de un mismo factor por sí mismo en una potencia (ej. $\\csc(x)\\cdot\\csc(x) \\to \\csc^2(x)$). No hace nada más — no usa identidades trigonométricas por su cuenta.' },
  { id:'comando-combinar', keywords:['combinar','comando combinar','que hace combinar','una sola fraccion','terminos semejantes'],
    answer:'"combinar" hace dos cosas: (1) une en una sola fracción los términos que se están sumando o restando en el nivel superior de tu expresión (si ya tienes una fracción y un término suelto, como en $1 + \\dfrac{\\operatorname{sen}^2(x)}{\\cos^2(x)}$, el término suelto se trata como si tuviera el mismo denominador; si hay DOS fracciones con denominadores distintos, primero hay que igualarlos con "amplificar"), y (2) agrupa y cancela/suma términos semejantes (idénticos, sin importar el orden de un producto) — ej. $-\\cot(x)\\csc(x) + \\csc(x)\\cot(x)$ se cancela a 0 porque es el mismo producto con signos opuestos.' },
