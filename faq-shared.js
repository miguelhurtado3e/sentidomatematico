...
const FAQ_BANK = [
  ...
  { id:'comando-sustituir', keywords:['sustituir','como sustituyo','flecha','->','como uso el comando sustituir','identidad conocida'],
    answer:'Para aplicar una identidad conocida a un término, escribe "expresión1 -> expresión2", ej: $\\tan(x) \\to \\dfrac{\\operatorname{sen}(x)}{\\cos(x)}$. El sistema solo lo acepta si ese par coincide con una identidad del catálogo (pregunta "catálogo de identidades") Y si esa expresión aparece de verdad en lo que tienes escrito — no puedes escribir la meta completa de una vez, tiene que ser un paso pequeño.' },
  { id:'comando-simplificar', keywords:['comando simplificar','que hace simplificar'],
    answer:'"simplificar" hace dos cosas automáticas: reparte una potencia sobre una fracción o un producto (ej. $(a/b)^2 \\to a^2/b^2$), y cancela una fracción cuando el numerador y el denominador son exactamente iguales (ej. $a/a \\to 1$). No hace nada más — no usa identidades trigonométricas por su cuenta.' },
  { id:'comando-combinar', keywords:['comando combinar','que hace combinar','una sola fraccion'],
    answer:'"combinar" une en una sola fracción los términos que se están sumando o restando en el nivel superior de tu expresión. Si ya tienes una fracción y un término suelto (como en $1 + \\dfrac{\\operatorname{sen}^2(x)}{\\cos^2(x)}$), el término suelto se trata como si tuviera el mismo denominador. Si hay DOS fracciones con denominadores distintos, primero hay que igualarlos con "amplificar".' },
  { id:'comando-amplificar', keywords:['comando amplificar','que hace amplificar'],
  ...
  { id:'comando-factorizar', keywords:['comando factorizar','que hace factorizar','factor comun'],
  ...
  { id:'comando-expandir', keywords:['comando expandir','que hace expandir','distribuir'],
  ...
  { id:'catalogo-identidades', keywords:['catalogo de identidades','que identidades conozco','lista de identidades','identidades conocidas','identidades disponibles'],
    answer:'El comando de sustitución (flecha ->) solo acepta estas identidades (y sus versiones al cuadrado), en cualquiera de los dos sentidos:\n\n• Cociente: $\\tan(x)=\\dfrac{\\operatorname{sen}(x)}{\\cos(x)}$, $\\cot(x)=\\dfrac{\\cos(x)}{\\operatorname{sen}(x)}$\n• Recíprocas: $\\sec(x)=\\dfrac{1}{\\cos(x)}$, $\\csc(x)=\\dfrac{1}{\\operatorname{sen}(x)}$, $\\cot(x)=\\dfrac{1}{\\tan(x)}$\n• Pitagóricas: $\\operatorname{sen}^2(x)+\\cos^2(x)=1$, $1+\\tan^2(x)=\\sec^2(x)$, $1+\\cot^2(x)=\\csc^2(x)$, y sus formas despejadas como $1-\\cos^2(x)=\\operatorname{sen}^2(x)$.' },
  { id:'nivel-1-identidades', keywords:['nivel 1','que es el nivel 1','meta','forma indicada','hasta donde debo llegar'],
    answer:'En el Nivel 1 no se demuestra una identidad con dos lados todavía: se te da una expresión y una META (ambas visibles desde el inicio), y debes transformar la expresión con los comandos hasta que quede EXACTAMENTE igual a la meta. Cuando lo logras, se grafican las dos (tu resultado y la meta) para confirmar visualmente que coinciden.' },
