...
const FAQ_BANK = [
  ...
  { id:'que-es-identidad', keywords:['que es una identidad','que es identidad','identidad','identidad trigonometrica','que significa identidad','definicion de identidad','que es demostrar una identidad'],
  ...
  { id:'que-es-funcion', keywords:['que es una funcion','que es funcion','funcion','definicion de funcion','funcion matematica','que es f(x)','que significa una funcion'],
  ...
  { id:'que-es-matriz', keywords:['que es una matriz','que es matriz','matriz','matrices','definicion de matriz','que es un elemento de una matriz','filas y columnas de una matriz'],
  ...
  { id:'transpuesta-matriz', keywords:['transpuesta','que es transpuesta','que es la transpuesta','transpuesta de una matriz','que es a transpuesta','matriz transpuesta','como se transpone'],
  ...
  { id:'matriz-aumentada', keywords:['matriz aumentada','que es matriz aumentada','que es una matriz aumentada','para que sirve la matriz aumentada','que es a dado b'],
  ...
  { id:'matriz-doble', keywords:['matriz doble','es matriz doble','que es una matriz doble','para que sirve el modo doble','rellenar b como identidad','a dado b doble'],
  ...
  { id:'sistema-ecuaciones-lineales', keywords:['sistema','que es sistema','sistema de ecuaciones','sistema de ecuaciones lineales','que es un sistema de ecuaciones'],
  ...
  { id:'metodo-gauss', keywords:['metodo de gauss','que es gauss','eliminacion gaussiana','que es el metodo gauss','forma escalonada'],
  ...
  { id:'metodo-gauss-jordan', keywords:['metodo de gauss jordan','que es gauss jordan','gauss-jordan','gauss jordan','forma escalonada reducida'],
  ...
  { id:'inversa-matriz', keywords:['inversa','que es inversa','que es la inversa','inversa de una matriz','como hallo la inversa','matriz inversa','a la menos uno'],
  ...
  { id:'que-es-pagina-sumas', keywords:['que es esta pagina','para que sirve sumas','area y pendiente como sumas','area bajo la curva por sumas','que hace sumas.html'],
    answer:'"Área y Pendiente como Sumas" te deja encontrar, SIN calculadora, el área bajo un polinomio y la pendiente de sus rectas tangentes — dos procesos inversos que comparten la misma idea: sumas telescópicas. En "Área" armas el área como una suma de rectángulos que tú sumas a mano; en "Pendiente" es al revés — primero lees cada pendiente (recta por recta), y de ahí sacas la suma de sus diferencias. Es una manera de entender de dónde salen el área bajo la curva y la derivada antes de estudiar cálculo.' },
  ...
  { id:'que-es-n-sumas', keywords:['que es n en sumas','limite superior','hasta donde llega el area','que significa n','que es n en pendiente'],
    answer:'$n$ es el mismo campo para las dos situaciones, con un significado distinto en cada una: en "Área" es el límite superior (se calcula el área entre $x=0$ y $x=n$); en "Pendiente" es cuántas rectas tangentes vas a tocar (en $x=1,2,...,n$). Debe ser un número entero, entre 1 y 12 en esta página.' },
  ...
  { id:'verificar-funcion-sumas', keywords:['verificar funcion','como formulo la funcion','que es a=n^2','tercer input','formula del area','a de n'],
    answer:'Después de acertar toda la sucesión de sumas parciales, se desbloquea un tercer campo: ahí formulas la FUNCIÓN que relaciona el índice $i$ con su suma parcial — la fórmula cerrada del área. Escríbela como $A=...$, $A(n)=...$, o solo la expresión, usando $n$ o $x$ como variable indistintamente (son intercambiables, igual que $x$/$t$ en Recta.html); debe ser un polinomio, igual de restringido que en el resto de la página. Por ejemplo, si tu sucesión fue $1, 4, 9, 16, 25$, la función es $A=n^2$. El botón "Verificar función" compara tu fórmula, coeficiente por coeficiente y de forma EXACTA, contra la fórmula real (la misma que generó los rectángulos) — si no coincide, no te dice cuál es la correcta, solo que sigas intentando. Acertarla NO da el punto todavía: desbloquea la Valoración estudiada (pregunta "valoración estudiada" para más detalle), que es donde realmente se gana.' },

  /* --- Sumas.html: situación "Pendiente" (rectas tangentes, proceso inverso a Área) --- */
  { id:'boton-pendiente-sumas', keywords:['boton pendiente','que hace el boton pendiente','para que sirve pendiente','que es la situacion pendiente'],
    answer:'El botón "Pendiente" arma la situación con el mismo $n$ que "Área": deja fija de inmediato la recta tangente REAL en $x=0$ (la "recta 0"), como punto de partida, y habilita el botón "Tocar siguiente recta" para ir revelando las demás rectas tangentes (en $x=1,2,...,n$) una por una. A diferencia de "Abrir llave", aquí no hay una sola animación automática — tú controlas el ritmo, recta por recta.' },
  { id:'tocar-siguiente-recta', keywords:['tocar siguiente recta','siguiente recta','boton tocar siguiente recta','como aparecen las rectas tangentes','se mueve la regla','regla que se desliza','por que se mueve la recta'],
    answer:'Cada clic en "Tocar siguiente recta" desliza una regla TANGENCIALMENTE por la curva, desde el punto anterior hasta el siguiente entero (ej. de $x=0$ a $x=1$), evaluando la curva en vivo mientras se mueve. Al llegar exactamente al entero, la regla se fija con sus 3 puntos de color. Las rectas ya tocadas quedan como referencia (punteadas, sin puntos); solo la última se ve sólida — así siempre es claro cuál estás estudiando en ese momento.' },
  { id:'recta-0-pendiente', keywords:['que es la recta 0','recta en x=0','por que aparece una recta antes de tocar nada','punto de partida pendiente','de donde sale el primer termino de la suma'],
    answer:'La "recta 0" es la recta tangente REAL en $x=0$ — aparece fija apenas presionas "Pendiente", sin que tengas que tocarla. Es el punto de partida del proceso, igual que Área asume que no hay área acumulada antes de $x=0$: aquí, en vez de asumir un valor, se muestra la recta de verdad para que leas su pendiente. Esa pendiente es la que necesitas para el primer término de "Tu suma de diferencias".' },
  { id:'colores-rectas-tangentes', keywords:['por que cambian de color las rectas','colores de las rectas tangentes','cada recta un color','se pone punteada la recta anterior','por que desaparecen los puntos'],
    answer:'Cada recta tangente tiene su propio color, para distinguirla y para relacionarla con sus 2 puntos auxiliares (del mismo color que su recta). Solo la recta que acabas de tocar (o la recta 0, si todavía no tocas ninguna) se ve sólida con sus 3 puntos; en cuanto tocas la siguiente, la anterior pasa automáticamente a verse punteada y sin puntos, para que el lienzo no se sature con demasiadas rectas a la vez.' },
  { id:'como-leer-pendiente-recta', keywords:['como leo la pendiente en la grafica','como se lee la pendiente','que significan los 2 puntos','punto a la izquierda y a la derecha','altura entre los puntos'],
    answer:'La recta activa (sólida) muestra 3 puntos de su color: el de tangencia ($x=i$) y uno a cada lado, a exactamente 1 unidad de distancia horizontal ($x=i-1$ y $x=i+1$). Como esa distancia horizontal siempre es 1, la diferencia de ALTURA entre el punto de tangencia y cualquiera de los otros dos ES la pendiente — léela directamente contando cuadros de la rejilla (acércate con el zoom si hace falta).' },
  { id:'verificar-suma-pendiente', keywords:['verificar suma pendiente','como escribo la suma de pendiente','suma de diferencias de pendientes','primer termino de la suma pendiente','que es m1 menos m0'],
    answer:'En "Tu suma de diferencias" escribes, término por término, la SUMA de los cambios entre pendientes consecutivas. El PRIMER término es la pendiente en $x=1$ MENOS la pendiente real que lees en la recta 0 ($x=0$); los siguientes son cada pendiente menos la anterior. A diferencia de Área, este campo y el de la sucesión se abren juntos desde que presionas "Pendiente" — pero "Verificar suma" solo confirma tu respuesta una vez que ya tocaste las $n$ rectas.' },
  { id:'sucesion-pendientes', keywords:['sucesion de pendientes','segundo input pendiente','verificar sucesion pendiente','como escribo la sucesion de pendientes'],
    answer:'En paralelo a la suma, escribes la SUCESIÓN de pendientes: el valor en $x=1, x=2, ..., x=n$, en orden y separados por comas ($n$ valores — la pendiente en $x=0$ no se incluye aquí, solo se usa para el primer término de la suma). "Verificar sucesión" marca cada valor en su propio óvalo, con su índice justo debajo.' },
  { id:'verificar-funcion-pendiente', keywords:['verificar funcion pendiente','como formulo la funcion pendiente','que es m=4x-1','formula de la pendiente','m de x','funcion pendientes'],
    answer:'Cuando la suma Y la sucesión quedan correctas, se desbloquea el tercer campo: ahí formulas la función $m(x)$ que da la pendiente para cualquier $x$ — la derivada, en forma cerrada. Escríbela como $m=...$, $m(x)=...$, o solo la expresión, usando $x$ (o $n$, son intercambiables) como variable; debe ser un polinomio. El botón "Verificar función" la compara, coeficiente por coeficiente y de forma EXACTA, contra la derivada real de tu función graficada. Acertarla NO da el punto todavía: desbloquea la Valoración estudiada (pregunta "valoración estudiada" para más detalle), que es donde realmente se gana.' },
  { id:'valoracion-estudiada-sumas', keywords:['valoracion estudiada','que es valorar la referencia estudiada','ultimo paso sumas','como gano el punto en sumas','cuando se otorga el punto','situaciones resueltas'],
